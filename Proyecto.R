...
OEE_DATA <- OEE_DATA_ORIGINAL #Hacer copia
  ...
  ### ----VALIDACION DE SUPUESTOS - REGRESION LINEAL MULTIPLE----
  library(dplyr)
  library(car)
  library(lmtest)
  library(ggplot2)

  # Crear tabla de resultados vacía
  ...
  # Lista de trenes
  trenes <- unique(BASE_MODELO$TREN)

  for (tren in trenes) {
    cat("\n\n----------------------------\n", "Análisis para tren:", tren, "\n----------------------------\n")

    # Subset
    df_tren <- BASE_MODELO %>% filter(TREN == tren)

    # Modelo
    modelo <- lm(KG ~ DISPONIBILIDAD + HORAS + Costo_prom_ent, data = df_tren)

    # Métricas
    r2 <- summary(modelo)$r.squared
    shapiro_p <- if (nrow(df_tren) < 5000) shapiro.test(resid(modelo))$p.value else NA
    bp_p <- bptest(modelo)$p.value
    vif_vals <- vif(modelo)
    vif_mean <- mean(vif_vals)
    ...
    plot(modelo$fitted.values, resid(modelo),
         main = paste("Residuos vs Ajustados\n", tren),
         xlab = "Ajustados", ylab = "Residuos")
    abline(h = 0, col = "red")

    # 2. Q-Q Plot
    qqnorm(resid(modelo), main = "Q-Q Plot")
    qqline(resid(modelo), col = "blue")

    # 3. ACF de residuos
    acf(resid(modelo), main = "ACF de residuos")

    # 4. VIF como gráfico
    barplot(vif_vals,
            main = "VIF por variable",
            horiz = TRUE, las = 1, col = "lightblue")
  ...
  #RESULTADOS VALIDACION DE SUPUSTOS - TABLA
  library(knitr)
  library(kableExtra)
  library(dplyr)
  ...
  tabla_resultados %>%
    arrange(TREN) %>%
    kable(
      caption = "Diagnóstico de supuestos estadísticos por tren (Regresión Lineal Múltiple)",
      col.names = c("Tren", "R²", "p (Shapiro-Wilk)", "p (Breusch-Pagan)", "VIF Promedio"),
      digits = 4,
      align = "c",
      format = "html"  # usa "markdown" si estás en consola o en .Rmd plano
    ) %>%
  ...
  # Entrenar modelo con búsqueda por grilla
  ...
  # Agregar predicciones a testData
  df_test_all <- testData %>%
    mutate(
      pred_xgb_default = pred_test_xgb,
      pred_xgb_tuned   = pred_test_xgb_tuned,
      pred_rf_default  = pred_test_rf,
      pred_rf_tuned    = pred_test_rf_tuned
    )

  # === Construir tabla por tren ===
  tabla_resultados_trenes <- df_test_all %>%
    group_by(TREN) %>%
    summarise(
      RMSE_XGB_def = sqrt(mean((KG - pred_xgb_default)^2, na.rm = TRUE)),
      R2_XGB_def   = 1 - sum((KG - pred_xgb_default)^2, na.rm = TRUE) / sum((KG - mean(KG, na.rm = TRUE))^2, na.rm = TRUE),

      RMSE_XGB_tuned = sqrt(mean((KG - pred_xgb_tuned)^2, na.rm = TRUE)),
      R2_XGB_tuned   = 1 - sum((KG - pred_xgb_tuned)^2, na.rm = TRUE) / sum((KG - mean(KG, na.rm = TRUE))^2, na.rm = TRUE),

      RMSE_RF_def = sqrt(mean((KG - pred_rf_default)^2, na.rm = TRUE)),
      R2_RF_def   = 1 - sum((KG - pred_rf_default)^2, na.rm = TRUE) / sum((KG - mean(KG, na.rm = TRUE))^2, na.rm = TRUE),

      RMSE_RF_tuned = sqrt(mean((KG - pred_rf_tuned)^2, na.rm = TRUE)),
      R2_RF_tuned   = 1 - sum((KG - pred_rf_tuned)^2, na.rm = TRUE) / sum((KG - mean(KG, na.rm = TRUE))^2, na.rm = TRUE)
    ) %>%
    arrange(TREN)
  ...
  tabla_resultados_trenes_red <- tabla_resultados_trenes %>%
    mutate(across(where(is.numeric), ~ round(.x, 2)))

  # Mostrar tabla con formato
  tabla_resultados_trenes_red %>%
    arrange(TREN) %>%
    kable(
      caption = "Comparación de desempeño por tren y configuración de modelo (XGBoost y Random Forest)",
      col.names = c("Tren",
                    "RMSE XGB (Def)", "R² XGB (Def)",
                    "RMSE XGB (Tuned)", "R² XGB (Tuned)",
                    "RMSE RF (Def)", "R² RF (Def)",
                    "RMSE RF (Tuned)", "R² RF (Tuned)"),
      align = "c",
      format = "html"  # usa "markdown" si estás fuera de RMarkdown
    ) %>%
    kable_styling(full_width = FALSE, bootstrap_options = c("striped", "hover"), font_size = 12)
  ...
  # XGBoost por defecto
  rmse_xgb_default <- sqrt(mean((testData$KG - pred_test_xgb)^2, na.rm = TRUE))
  r2_xgb_default <- 1 - sum((testData$KG - pred_test_xgb)^2, na.rm = TRUE) / sum((testData$KG - mean(testData$KG, na.rm = TRUE))^2, na.rm = TRUE)

  # XGBoost ajustado
  pred_test_xgb_tuned <- predict(modelo_xgb_caret, newdata = testData)
  rmse_xgb_tuned <- sqrt(mean((testData$KG - pred_test_xgb_tuned)^2, na.rm = TRUE))
  r2_xgb_tuned <- 1 - sum((testData$KG - pred_test_xgb_tuned)^2, na.rm = TRUE) / sum((testData$KG - mean(testData$KG, na.rm = TRUE))^2, na.rm = TRUE)

  # === MÉTRICAS PARA RANDOM FOREST ===

  # Random Forest por defecto
  rmse_rf_default <- sqrt(mean((testData$KG - pred_test_rf)^2, na.rm = TRUE))
  r2_rf_default <- 1 - sum((testData$KG - pred_test_rf)^2, na.rm = TRUE) / sum((testData$KG - mean(testData$KG, na.rm = TRUE))^2, na.rm = TRUE)

  # Random Forest ajustado
  pred_test_rf_tuned <- predict(modelo_rf_caret, newdata = testData)
  rmse_rf_tuned <- sqrt(mean((testData$KG - pred_test_rf_tuned)^2, na.rm = TRUE))
  r2_rf_tuned <- 1 - sum((testData$KG - pred_test_rf_tuned)^2, na.rm = TRUE) / sum((testData$KG - mean(testData$KG, na.rm = TRUE))^2, na.rm = TRUE)

  # === CÁLCULO DE MEJORAS ===
  mejora_xgb <- (rmse_xgb_default - rmse_xgb_tuned) / rmse_xgb_default * 100
  mejora_rf <- (rmse_rf_default - rmse_rf_tuned) / rmse_rf_default * 100

  # === IMPRIMIR RESULTADOS COMPARATIVOS ===
  cat("\nXGBoost - Por defecto:     RMSE =", rmse_xgb_default, ", R² =", r2_xgb_default)
  cat("\nXGBoost - Ajustado:        RMSE =", rmse_xgb_tuned, ", R² =", r2_xgb_tuned)
  cat("\n--> Mejora RMSE XGBoost:   ", round(mejora_xgb, 2), "%")

  cat("\n\nRandom Forest - Por defecto: RMSE =", rmse_rf_default, ", R² =", r2_rf_default)
  cat("\nRandom Forest - Ajustado:    RMSE =", rmse_rf_tuned, ", R² =", r2_rf_tuned)
  cat("\n--> Mejora RMSE RF:         ", round(mejora_rf, 2), "%")
  ...
  # Ordenar por tren y mostrar tabla bonita
  resultados_trenes_combinados %>%
    arrange(TREN) %>%
    kable(
      caption = "Tabla: Desempeño de modelos por tren productivo",
      col.names = c("Tren", "Modelo", "R²", "RMSE", "MAE", "MAPE"),
      digits = 3,
      align = "c"
    ) %>%
    kable_styling(full_width = FALSE, bootstrap_options = c("striped", "hover"), position = "center", font_size = 12)
    ...
    #Boxplot de errores absolutos por modelo y tren
      ...
      predicciones_rf <- datos_rf %>% select(Fecha, TREN, Prediccion = Capacidad_Max) %>% mutate(Modelo = "RF")
      ...
      tabla_capacidades_final %>%
        mutate(Capacidad_Proyectada_MT = round(Capacidad_Proyectada_MT, 2)) %>%
